...
// NinjaProcess do the ninja build process:
// 1. apply resources.
// 2. keep heartbeat.
// 3. execute the ninja command when resources are ready.
// 4. release the resources when cancel or finish.
func NinjaProcess(c *commandCli.Context) error {
	initProcess(c)
	return ninjaProcess(c)
}
...
func ninjaProcess(c *commandCli.Context) error {
	...
	task, ok, err := applyDistCCResources(types.DistccServerSets{
		CommandType:   types.CommandNinja,
		ProjectId:     projectID,
		BuildId:       buildID,
	...
	if ok && err != nil {
		...
		return localNinjaCompiling(args)
	} else if !ok {
...
func localNinjaCompiling(param string) error {
	if NoLocal {
		return ErrNoLocal
	}

	if hookMode {
		return localNinjaCompilingHook(param)
	}
	...
	switch Compiler {
	case CompilerGcc:
		command = fmt.Sprintf("BK_CC='gcc' BK_CXX='g++' BK_JOBS=%d ninja -j%d %s", jobs, jobs, param)
	case CompilerClang:
		command = fmt.Sprintf("BK_CC='clang' BK_CXX='clang++' BK_JOBS=%d ninja -j%d %s", jobs, jobs, param)
	}
...
func localNinjaCompilingHook(param string) error {
	var command string
	command = fmt.Sprintf("ninja %s", param)
	fmt.Printf("exec command: %s\n", command)

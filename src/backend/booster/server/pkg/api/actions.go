...
var (
	...
	apiV2actions = make([]*httpserver.Action, 0, 100)
)
...
// RegisterV2Action register a handler into v1 actions
// means all the URL of these handlers are start with PathV1
func RegisterV2Action(action Action) {
	apiV2actions = append(apiV2actions, httpserver.NewAction(action.Verb, action.Path, action.Params, action.Handler))
}

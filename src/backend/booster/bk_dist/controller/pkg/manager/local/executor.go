...
func (e *executor) executeLocalTask() *types.LocalTaskExecuteResult {
	blog.Debugf("executor: try to execute local-task from pid(%d) command:[%s]", e.req.Pid, strings.Join(e.req.Commands, " "))
	defer e.mgr.work.Basic().UpdateJobStats(e.stats)
	...
	blog.Debugf("executor: success to execute local-task from pid(%d) command:[%s]", e.req.Pid, strings.Join(e.req.Commands, " "))
	return &types.LocalTaskExecuteResult{
		Result: &dcSDK.LocalTaskResult{
			ExitCode: code,
			Stdout:   stdout,

...
# for uba files
echo "4. install uba clients"
ubacli="UbaCli"
ubaAgent="UbaAgent"
detourslib="libUbaDetours.so"
hostlib="libUbaHost.so"
if [ ! -f ${current_dir}/${ubacli} -o ! -f ${current_dir}/${ubaAgent} -o ! -f ${current_dir}/${detourslib} -o ! -f ${current_dir}/${hostlib} ];then
        echo "not found ${current_dir}/${ubacli} or ${current_dir}/${ubaAgent} or ${current_dir}/${detourslib} or ${current_dir}/${hostlib}, please
 ensure these files existed "
        cd ${olddir}
        exit 1
fi

if [ -f ${userbin}/${ubacli} ];then
        mv ${userbin}/${ubacli} ${userbin}/${ubacli}.bak
fi
cp ${current_dir}/${ubacli} ${userbin}/

if [ -f ${userbin}/${ubaAgent} ];then
        mv ${userbin}/${ubaAgent} ${userbin}/${ubaAgent}.bak
fi
cp ${current_dir}/${ubaAgent} ${userbin}/

if [ -f ${userbin}/${detourslib} ];then
        mv ${userbin}/${detourslib} ${userbin}/${detourslib}.bak
fi
cp ${current_dir}/${detourslib} ${userbin}/

if [ -f ${userbin}/${hostlib} ];then
        mv ${userbin}/${hostlib} ${userbin}/${hostlib}.bak
fi
cp ${current_dir}/${hostlib} ${userbin}/

# install hook so
echo "5. install bk hook so"
soname="bk-hook.so"
...
echo "6. install hook and uba config files"
